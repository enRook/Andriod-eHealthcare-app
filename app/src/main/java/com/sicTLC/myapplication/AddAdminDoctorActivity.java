...
public class AddAdminDoctorActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_admin_doctor);
        ...
        addButton.setOnClickListener(view -> validateInputs());


        TextView gotoAdminTextView = findViewById(R.id.gotoAdminTextView);
        gotoAdminTextView.setOnClickListener(view -> {
            startActivity(new Intent(AddAdminDoctorActivity.this, AdminActivity.class));
finish();
        });
    ...
    private void saveUserToDatabase(String username, String firstName, String lastName,
        ...
        if (userDBHelper.doesUsernameExist(username)) {
            ...
            if (userId != -1) {
                Log.d("AddAdminDoctorActivity", "User registered successfully with ID: " + userId);
                Toast.makeText(getApplicationContext(), "User registered successfully!", Toast.LENGTH_SHORT).show();
            } else {
                Log.e("AddAdminDoctorActivity", "Failed to register user");
                Toast.makeText(getApplicationContext(), "Failed to register user", Toast.LENGTH_SHORT).show();
            }
        }
    }

    // Your other methods (showDatePickerDialog, calculateAge) remain unchanged.
    private void showDatePickerDialog() {
        ...
        // Prevent the dialog from closing on date selection
